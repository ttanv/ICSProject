"""OPC UA protocol handler for metadata-first augmentation."""
...
class OpcUaProtocolHandler:
    """Emit OPC UA connection artifacts before generic grouping."""
    ...
    def build_artifacts(self, context: ProtocolBuildContext) -> ProtocolBuildResult:
        ...
        for indexed in iterable:
            ...
            if not indexed.records:
                continue
            if not context.augmentor._is_opcua_indexed_connection(indexed):
                continue

            orientation = context.augmentor._orient_opcua_connection(indexed)
