"""Export OPC UA GECO alerts into Cypher for Neo4j import."""
...
def _alert_guid(alert: GecoAlertEvent) -> str:
    raw = (
        f"GECOAlert|opcua|{alert.target_guid}|{alert.start_timestamp:.6f}|"
...
def render_alert_statements(result: GecoScoreResult) -> List[str]:
    """Return Cypher statements that materialize OPC UA GECO alerts."""
    statements: List[str] = []
