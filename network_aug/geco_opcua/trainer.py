"""Training logic for GECO-style OPC UA signal models."""
...
class GecoTrainConfig:
    """Configuration for offline OPC UA GECO training."""
...
def _fit_best_model_for_target(
    *,
    ...
    return GecoSignalModel(
        ...
        node_id=target.meta.node_id,
        display_name=target.meta.display_name,
...
def train_geco(config: GecoTrainConfig) -> GecoModelSet:
    """Train per-signal GECO-style models from baseline OPC UA signal data."""
    ...
    for server_host, series_list in sorted(device_groups.items()):
        logger.info(
            "Training OPC UA GECO models for server=%s (%d signals)",
            server_host,
            len(series_list),
        )
        for target in sorted(series_list, key=lambda item: item.meta.guid):
            model = _fit_best_model_for_target(
                ...
                preferred_predictors=invariant_adjacency.get(target.meta.guid, []),
                config=config,
            )
            if model is None:
    ...
    logger.info("Wrote %d OPC UA GECO models to %s", len(model_set.models), config.output)
    return model_set
