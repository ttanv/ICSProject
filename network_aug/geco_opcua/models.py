"""Serializable model and alert structures for GECO-style scoring (OPC UA).

Mirrors network_aug.geco.models but identifies signals by node_id/display_name
instead of register_address/unit_id.
"""
...
class GecoSignalModel:
    """Learned state-transition model for one OPC UA target signal."""
...
@dataclass
class GecoModelSet:
    """Collection of learned GECO-style OPC UA signal models."""
...
class GecoAlertEvent:
    """One alert episode for an OPC UA signal model."""
...
class GecoScoreResult:
    """Scoring output for one OPC UA GECO model set against a signal database."""
