...
LOG_DIR="logs/extracted_1hr/Fuxnet (Malware)_newversion"
PCAP_DIR="pcap_traffic/extracted_1hr/Fuxnet (Malware)_newversion"
OUT_DIR="paper_graphs_v1/Fuxnet(Malware)_newversion"
ASSETS="ICSGraph/Collection/assets.yaml"
TMP_DIR="$OUT_DIR/tmp"
RUN_LOG="$OUT_DIR/Fuxnet_newversion_1hr_extract.log"

BASE_CYPHER="$OUT_DIR/base_Fuxnet(Malware)_newversion.cypher"
AUG_CYPHER="$OUT_DIR/augmented_Fuxnet(Malware)_newversion.cypher"
MAIN_DB="$OUT_DIR/Fuxnet(Malware)_newversion_signals.duckdb"
MQTT_DB="$OUT_DIR/Fuxnet(Malware)_newversion_mqtt_signals.duckdb"
OPCUA_DB="$OUT_DIR/Fuxnet(Malware)_newversion_opcua_signals.duckdb"
CACHE="$OUT_DIR/Fuxnet_newversion_pcap_index.pkl"
PROTOCOL_CACHE="$OUT_DIR/Fuxnet_newversion_protocol_signal_extract_cache.pkl"
...
main() {
  ...
  {
    log "Starting Fuxnet(Malware)_newversion 1hr extraction into $OUT_DIR"
    log "Logs: $LOG_DIR"
    log "PCAPs: $PCAP_DIR"
    log "TMPDIR: $TMP_DIR"
    ...
    log "Fuxnet(Malware)_newversion 1hr extraction complete"
  } >> "$RUN_LOG" 2>&1
}
